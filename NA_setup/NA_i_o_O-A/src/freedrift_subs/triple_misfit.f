...
c 
     ...
     &          model, 
     & 		uwind, vwind, wind_mag_in, wind_sig,
     & 		uo_i, vo_i, o_i_mag_in, o_i_sig,
     & 		hifc, 
...
c
        real            model(4)
     	real   		uwind, vwind, wind_mag_in, wind_sig
     	real   		uo_i, vo_i, o_i_mag_in, o_i_sig
     	real   		windf_mag, o_i_mag
...
c constants used to solve free drift balance
	wind_mag = HYPOT(uwind,vwind)
	o_i_mag = HYPOT(uo_i,vo_i)
...
c   		x =   thA - ATAN2(Eq11*yi + Eq12,yi*Eq13)
...
c           x = - thA + ATAN(Eq11 + Eq12/(yi*Eq13))
    	    x = - thA + ATAN2(Eq11*(yi*Eq13) + Eq12,(yi*Eq13))
c   	    x =   thA - ATAN2(Eq11*yi + Eq12,yi*Eq13)
c           write(*,*)' yi = ',yi,' fi/dfi = ',fi/dfi
c            write(*,*)' x  = ',x
